...
def _display_evaluation_tab(project_root: Path) -> None:
    ...
    st.write(
        "This section only displays measured results that were generated by the repository scripts. "
        "It does not show unverified scores or template comparison numbers."
    )

    summary_path = project_root / "reports" / "evaluation_summary.csv"
    ...
    if not found_any:
        st.info(
            "No benchmark result files were found yet. The live app is functional, but formal accuracy/F1 results "
            "only appear here after running the evaluation script on a labeled CSV."
        )
        st.markdown(
            "**Expected benchmark CSV columns:** `claim,label`  "
            "\n\n"
            "**Output files:** `reports/benchmark_metrics.json`, `reports/evaluation_summary.csv`, and optional figures."
        )

    with st.expander("Developer command to generate benchmark files", expanded=False):
        st.code(
        ...
        st.caption(
            "Use this locally or in a development environment after adding a labeled benchmark CSV. "
            "The app will automatically display the saved results when those files exist."
        )
