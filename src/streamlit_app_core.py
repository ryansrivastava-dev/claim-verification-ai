...
def _display_evaluation_tab(project_root: Path) -> None:
    ...
    st.write(
        ...
        "It does not show placeholder scores or fake leaderboard numbers."
    )
    ...
    if not found_any:
        ...
        st.markdown(
            ...
            "\n\n"
            "**Output files:** `reports/benchmark_metrics.json`, `reports/evaluation_leaderboard.csv`, and optional figures."
        )
...
def _display_architecture_tab(project_root: Path) -> None:
    ...
    st.write(
        ...
        "handles current/time-sensitive claims internally, and exports a transparent report."
    )
