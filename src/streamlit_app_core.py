...
def _display_current_context(result: dict) -> None:
    """Render current/time-sensitive handling in plain language when it matters."""
    profile = result.get("claim_profile", {})
    if not profile.get("needs_current_source"):
        return

    st.markdown("### Current-source handling")
    st.write(
        "This claim appears time-sensitive, so the system gives more weight to current, official, "
        "or role-specific sources and avoids treating old biography matches as proof of present-day facts."
    )
...
def _display_evidence(evidence: list[dict]) -> None:
    ...
    for index, item in enumerate(evidence, start=1):
        with st.container(border=True):
            ...
            entailment_label = item.get("entailment_label")
            if entailment_label and entailment_label != "not_checked":
                st.write(
                    "**Claim-evidence check:** "
                    f"{str(entailment_label).title()} "
                    f"({float(item.get('entailment_confidence', 0.0)):.2f})"
                )
...
def _display_clean_technical_details(result: dict) -> None:
    ...
    if profile.get("needs_current_source"):
        st.markdown("**Current-source handling:** Applied for time-sensitive wording")
    ...
    detail_rows = [
        {
            ...
            "entailment_label": item.get("entailment_label"),
            "entailment_confidence": item.get("entailment_confidence"),
...
def _read_json(path: Path) -> dict:
    """Read a JSON file safely for display."""
    import json
...
def _display_metric_cards(metrics: dict) -> None:
    """Show compact metric cards without hardcoding any results."""
    if not metrics:
        return
    cards = []
    if "num_claims" in metrics:
        cards.append(("Claims evaluated", metrics.get("num_claims")))
    if "num_pairs" in metrics:
        cards.append(("Pairs evaluated", metrics.get("num_pairs")))
    for key, label in [
        ("accuracy", "Accuracy"),
        ("macro_f1", "Macro F1"),
        ...
        if key in metrics:
            ...
            cards.append((label, f"{float(value):.3f}" if isinstance(value, (int, float)) else value))
...
def _display_evaluation_tab(project_root: Path) -> None:
    ...
    st.write(
        "This page displays only measured outputs created by the repository's evaluation scripts. "
        "If a result file is missing, that section stays empty instead of showing placeholder scores."
    )

    reports_dir = project_root / "reports"
    figures_dir = reports_dir / "figures"

    benchmark_metrics_path = reports_dir / "benchmark_metrics.json"
    benchmark_predictions_path = reports_dir / "benchmark_predictions.csv"
    ...
    ablation_path = reports_dir / "ablation_results.csv"
    significance_path = reports_dir / "significance_tests.csv"
    ablation_chart_path = figures_dir / "ablation_macro_f1.png"

    entailment_metrics_path = reports_dir / "entailment_metrics.json"
    entailment_predictions_path = reports_dir / "entailment_predictions.csv"
    entailment_confusion_path = figures_dir / "entailment_confusion_matrix.png"

    calibration_path = reports_dir / "confidence_calibration.csv"
    calibration_chart_path = figures_dir / "confidence_calibration.png"
    ...
    with result_tabs[0]:
        if benchmark_metrics_path.exists():
            metrics = _read_json(benchmark_metrics_path)
            _display_metric_cards(metrics)
        if evaluation_summary_path.exists():
            st.subheader("Summary table")
            st.dataframe(pd.read_csv(evaluation_summary_path), use_container_width=True)
        if classification_report_path.exists():
            st.subheader("Per-class report")
            st.dataframe(pd.read_csv(classification_report_path), use_container_width=True)
        if benchmark_predictions_path.exists():
            with st.expander("Prediction-level results", expanded=False):
                st.dataframe(pd.read_csv(benchmark_predictions_path), use_container_width=True)
        if confusion_path.exists():
            st.subheader("Confusion matrix")
            st.image(str(confusion_path), use_container_width=True)
        if not any(path.exists() for path in [benchmark_metrics_path, evaluation_summary_path, classification_report_path, benchmark_predictions_path, confusion_path]):
            st.info("No verdict benchmark files found yet. Run the benchmark script to populate this section.")

    with result_tabs[1]:
        if ablation_path.exists():
            st.subheader("Retrieval and credibility ablation")
            st.dataframe(pd.read_csv(ablation_path), use_container_width=True)
        if significance_path.exists():
            st.subheader("McNemar significance tests")
            st.write(
                "These tests compare paired correctness between the BM25 baseline and each alternative configuration. "
                "They help show whether an improvement is likely larger than random variation on the evaluated sample."
            )
            st.dataframe(pd.read_csv(significance_path), use_container_width=True)
        if ablation_chart_path.exists():
            st.image(str(ablation_chart_path), use_container_width=True)
        if not any(path.exists() for path in [ablation_path, significance_path, ablation_chart_path]):
            st.info("No ablation files found yet. Run the ablation script to compare TF-IDF, BM25, hybrid alpha values, and source credibility weighting.")

    with result_tabs[2]:
        if entailment_metrics_path.exists():
            metrics = _read_json(entailment_metrics_path)
            _display_metric_cards(metrics)
        if entailment_predictions_path.exists():
            st.subheader("Entailment predictions")
            st.dataframe(pd.read_csv(entailment_predictions_path), use_container_width=True)
        if entailment_confusion_path.exists():
            st.subheader("Entailment confusion matrix")
            st.image(str(entailment_confusion_path), use_container_width=True)
        if not any(path.exists() for path in [entailment_metrics_path, entailment_predictions_path, entailment_confusion_path]):
            st.info("No entailment evaluation files found yet. Run the entailment evaluation script to test support/contradiction/neutral decisions.")

    with result_tabs[3]:
        if calibration_path.exists():
            st.subheader("Confidence calibration table")
            st.dataframe(pd.read_csv(calibration_path), use_container_width=True)
        if calibration_chart_path.exists():
            st.image(str(calibration_chart_path), use_container_width=True)
        if not any(path.exists() for path in [calibration_path, calibration_chart_path]):
            st.info("No calibration files found yet. Run calibration after generating benchmark predictions.")

    with result_tabs[4]:
        st.subheader("Commands to generate real results")
        st.write("Run these locally after installing dependencies. The app will display the saved files automatically after you commit them.")
        st.code(
        ...
        st.caption(
            "The bundled benchmark CSV is a small sanity-check set. For a paper, also run a larger public benchmark such as SciFact, FEVER, or AVeriTeC and report the setup clearly."
        )
...
def _display_architecture_tab(project_root: Path) -> None:
    ...
    if diagram_path.exists():
        ...
        st.code(
            ...
            "Evidence Synthesis → Current-Context Handling → Verdict + Citations + Report"
        )
    ...
    st.write(
        ...
        "uses an entailment check to avoid keyword-only support, handles current/time-sensitive claims internally, and exports a transparent report."
    )
...
def _queue_verification() -> None:
    """Allow Ctrl+Enter/apply in the text area to trigger verification on rerun."""
    st.session_state["verify_requested"] = True
...
def _live_fact_check_tab() -> None:
    ...
    claim = st.text_area(
        ...
        height=110,
        key="claim_to_verify",
        help="Press Ctrl+Enter while typing or click Verify claim to run verification.",
        on_change=_queue_verification,
    )
    ...
    run_requested = run_button or bool(st.session_state.pop("verify_requested", False))
    ...
    if run_requested:
        ...
        with tabs[0]:
            ...
            _display_current_context(result)
...
def main(project_root: Path | None = None) -> None:
    ...
    st.write(
        ...
        "summarizes findings, checks claim-evidence entailment, handles time-sensitive claims, and generates citation-backed reports."
    )
