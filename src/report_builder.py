...
def build_markdown_report(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *( [f"**Current-source handling:** Applied"] if profile.get("needs_current_source") else [] ),
        "",
        "## Explanation",
    ...
    for index, item in enumerate(evidence, start=1):
        ...
        lines.extend(
            [
                ...
                f"- Claim-evidence check: {_safe(item.get('entailment_label', 'not checked'))} ({float(item.get('entailment_confidence', 0.0)):.2f})",
                f"- Score: {score:.3f}",
                f"- Summary: {summary}",
            ]
        )
